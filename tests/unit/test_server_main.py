...
class TestServerMain:
    ...
    def test_server_main_stdin_mode_valid_tool_call(self, mock_server_run, mock_stdin):
        ...
        with patch('cowsay_mcp.server.run_cowsay', return_value='Mocked ASCII art') as mock_run_cowsay:
    ...
    def test_server_main_stdin_mode_missing_args(self, mock_server_run, mock_stdin):
        ...
        with patch('cowsay_mcp.server.run_cowsay', return_value='ASCII art') as mock_run_cowsay:
            with patch('builtins.print') as mock_print:
                server_main()

        mock_run_cowsay.assert_called_once_with("")  # Should pass empty string for missing text

    @patch('sys.stdin')
    @patch('cowsay_mcp.main.server.run')
    def test_server_main_stdin_mode_tool_execution_error(self, mock_server_run, mock_stdin):
        """Test server handles tool execution errors."""
        mock_stdin.isatty.return_value = False
        tool_call = {"tool": "cowsay-mcp", "args": {"text": "test"}}
        mock_stdin.read.return_value = json.dumps(tool_call)

        with patch('cowsay_mcp.server.run_cowsay', side_effect=Exception("Tool failed")) as mock_run_cowsay:
            with patch('builtins.print') as mock_print:
                with pytest.raises(SystemExit):
                    server_main()
